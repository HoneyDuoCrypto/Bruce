#!/bin/bash
# Cloudflare-Only HDW Setup
# Professional setup with hdw.honey-duo.com access

HDW_PROJECT_DIR="$HOME/hdw_setup/honey_duo_wealth"
DOMAIN="honey-duo.com"
SUBDOMAIN="hdw"
TUNNEL_NAME="hdw-tunnel"

echo "🍯 HDW Professional Setup - Cloudflare Only"
echo "==========================================="
echo "🌐 Domain: $SUBDOMAIN.$DOMAIN"
echo "🔐 Secure tunnel with authentication"
echo ""

# Check project directory
if [ ! -d "$HDW_PROJECT_DIR" ]; then
    echo "❌ HDW project directory not found: $HDW_PROJECT_DIR"
    exit 1
fi
...
python3 -c "import flask" 2>/dev/null || {
    ...
    pip3 install flask --user || sudo apt install python3-flask -y
}
...
if ! command -v cloudflared &> /dev/null; then
    ...
    if ! command -v cloudflared &> /dev/null; then
        ...
        echo "Please install manually from: https://developers.cloudflare.com/cloudflare-one/connections/connect-apps/install-and-setup/installation/"
        exit 1
    fi
fi
...
# Step 2: Create secure web interface
echo "🔐 Creating secure web interface..."

cat > hdw_secure.py << 'WEB_EOF'
#!/usr/bin/env python3
"""
...
chmod +x hdw_secure.py

echo "✅ Secure web interface created"
...
echo "Make sure you're logged into your honey-duo.com Cloudflare account"
echo ""
...
if [ $? -ne 0 ]; then
    ...
    echo "Please ensure you're logged into Cloudflare and try again"
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "⚠️  DNS record creation failed"
    echo "💡 Please add manually in Cloudflare dashboard:"
    echo "   Type: CNAME"
    echo "   Name: $SUBDOMAIN"
    echo "   Content: $TUNNEL_ID.cfargotunnel.com"
    echo ""
    read -p "Press Enter after adding the DNS record manually..."
else
...
sudo tee /etc/systemd/system/hdw-tunnel.service > /dev/null << EOF
[Unit]
Description=HDW Cloudflare Tunnel
...
sudo systemctl enable hdw-tunnel.service

# Step 8: Create startup script
echo ""
echo "🚀 Creating startup script..."

cat > start_hdw.sh << 'START_EOF'
#!/bin/bash
...
chmod +x start_hdw.sh

# Step 9: Create reports directory
mkdir -p chatgpt_reports

echo ""
echo "🎉 HDW Professional Setup Complete!"
echo "==================================="
echo ""
echo "🌐 Your professional interface is ready:"
echo "   🔗 https://$SUBDOMAIN.$DOMAIN"
echo ""
echo "🔐 Login Credentials:"
echo "   Username: hdw"
echo "   Password: HoneyDuo2025!"
echo ""
echo "🚀 To start your interface:"
echo "   ./start_hdw.sh"
echo ""
echo "💡 Features:"
echo "   ✅ Professional design with animations"
echo "   ✅ Secure HTTPS access worldwide"
echo "   ✅ Authentication protection"
echo "   ✅ Mobile-responsive interface"
echo "   ✅ ChatGPT report generation"
echo "   ✅ Auto-starts on boot"
echo ""
echo "🔒 Security:"
echo "   ✅ No open ports on your router"
echo "   ✅ Encrypted tunnel via Cloudflare"
echo "   ✅ Login required for access"
echo ""

# Ask to start now
read -p "🎯 Start HDW interface now? (y/N): " start_now
